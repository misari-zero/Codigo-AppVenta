...
function formatRepo(repo) {
    ...
    var option = $(
        ...
        '<b>PVP:</b> <span class="badge badge-warning">S/.' + repo.pvp + '</span>' +
        '</p>' +
        '</div>' +
        '</div>' +
        '</div>');
...
$(function () {
    ...
    $('#tblProducts tbody')
        ...
        .on('change', 'input[name="cant"]', function () {
            ...
            vents.calculate_invoice();
            $('td:eq(5)', tblProducts.row(tr.row).node()).html('S/.' + vents.items.products[tr.row].subtotal.toFixed(2));
        });
    ...
    // Esto se puso aqui para que funcione bien el editar y calcule bien los valores del igv. // sino tomaría el valor del igv de la base debe
        ...
        success:function (res){
            if(res.respuesta==true){
                if(parseInt(res.mensaje)<10){
                    $("#nfacturas").html("00000000"+res.mensaje);
                }else if(parseInt(res.mensaje)<100){
                    $("#nfacturas").html("0000000"+res.mensaje);
                }
            }
        }
    });
